...
export default function FlashStage({ gameId, icon, name, accentVar, lang, t, children, onRulesOpenChange, rulesReaderNames, isHost, stageLaunchAt, onHostOpen }) {
  ...
  // déclencher le flash, chez tout le monde y compris l'hôte. Correctif
  // URGENT 2026-07 (dérive d'horloge entre machines, voir DoorStage.js) :
  // attente BORNÉE par SYNC_MAX_WAIT_MS, et un déclenchement reçu en direct
  // (`justLaunched`) anime toujours. Un client en retard (rejoint/rechargé
  // après coup) saute directement à 'open'.
