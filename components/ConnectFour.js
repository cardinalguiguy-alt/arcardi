...
export default function ConnectFour({ room, me, isHost, players, t, lang, onFinish }) {
  ...
  const stateRef = useRef({ board, turn, p1, p2, winner });
  ...
  useEffect(() => {
    stateRef.current = { board, turn, p1, p2, winner };
  }, [board, turn, p1, p2, winner]);
  ...
  function hostHandleMove({ by, col }) {
    ...
    board2[r][col] = s.turn;
    const won = checkWin(board2, r, col, s.turn);
    const full = !won && isBoardFull(board2);
    const winner2 = won ? s.turn : (full ? "draw" : null);
    const nextTurn = s.turn === "p1" ? "p2" : "p1";

    channelRef.current.send({
      type: "broadcast", event: "state",
      payload: { board: board2, turn: winner2 ? s.turn : nextTurn, winner: winner2, lastMove: { r, c: col } }
    });
  ...
  if (phase === "finished") {
    ...
    content = (
      <div>
        ...
        <p className="muted" style={{ textAlign: "center", marginBottom: 12, minHeight: 18 }}>
          {winner ? (
            winner === "draw" ? t("c4Draw")
              : isPlayer ? ((winner === "p1" && amP1) || (winner === "p2" && amP2) ? t("c4WinYou") : t("c4WinOpponent"))
              : `${winner === "p1" ? p1?.username : p2?.username} ${t("c4WinSpectator")}`
          ...
          {Array.from({ length: COLS }).map((_, c) => (
            ...
            >
              {Array.from({ length: ROWS }).map((_, r) => {
                ...
                return <div key={r} className={"c4-cell" + (val ? " filled " + val : "") + (isLast ? " last" : "")} />;
              })}
            </div>
          ))}
        </div>
