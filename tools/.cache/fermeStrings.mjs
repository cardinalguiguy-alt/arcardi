...
export const FERME_STR = {
  fr: {
    ...
    // 2026-09-12 : revente d'un cheval possédé, depuis la boutique.
    shopHorseOwnedHeader: "Chevaux possédés",
    horseCoatName: (coat) => coat === "black" ? "Cheval noir" : coat === "white" ? "Cheval blanc" : "Cheval bai",
    horseOwnedSub: (price) => `Payé ${price} or à l'achat.`,
    horseMountedTag: "Monté — indisponible à la vente",
    sellHorseBtn: (price) => `Vendre (${price} or)`,
    ...
    toolUsage: {
      ...
      scythe: "Taille les buissons sauvages (on les traverse alors sans ralentir), puis retire un buisson taillé pour libérer la case et récupérer quelques branchages.",
    },
    ...
    btnChat: "💬 Chat",
    btnChangeFarm: "🚜 Changer de ferme",
    btnLeave: "Quitter",
    ...
    devTeleportName: (k) => ({
      ...
      townChurch: "⛪ Valley Town — l'église (parvis)",   // 2026-09-20 : le sprite bitmap n'avait aucun arrêt direct
    ...
    // 2026-09-26 — le chat qu'on nourrit, l'épuisette et le carnet.
    catName: (c) => ({ roux: "le chat roux", noir: "le chat noir", tricolore: "la chatte tricolore" })[c] || "le chat",
    catMilkToast: (nm, n, max) => n >= max ? `🥛 ${nm[0].toUpperCase() + nm.slice(1)} lape son lait, et se frotte à vous.` : `🥛 ${nm[0].toUpperCase() + nm.slice(1)} lape le lait jusqu'à la dernière goutte. (${n}/${max})`,
    catAdoptedToast: (nm) => `💛 ${nm[0].toUpperCase() + nm.slice(1)} vous a adopté. Il viendra vous voir en ville.`,
    catGiftToast: (nm, fish) => `🐟 ${nm[0].toUpperCase() + nm.slice(1)} dépose un ${fish.toLowerCase()} à vos pieds. Merci… ?`,
    catMilkToday: (nm) => `${nm[0].toUpperCase() + nm.slice(1)} a déjà eu son lait aujourd'hui.`,
    catMilkNone: "Il vous faudrait du lait (de vache ou de chèvre).",
    promptCatMilk: "[E] Donner du lait au chat",
    promptNetBfly: "[E] Coup d'épuisette (papillon)",
    promptNetCarp: "[E] Coup d'épuisette (carpe)",
    netBoughtToast: (g) => `🥅 Une épuisette ! (−${g} or). Approchez-vous d'un papillon ou d'une carpe, puis E.`,
    netHaveToast: "Vous avez déjà une épuisette.",
    netNoGoldToast: (g) => `Il faut ${g} or dans la caisse pour l'épuisette.`,
    netMissToast: (carp) => carp ? "💦 Raté — la carpe file au fond." : "💨 Raté — il vous échappe.",
    netCarpToast: (n) => `🐟 Une carpe dans l'épuisette ! Relâchée aussitôt — la mairie en interdit la pêche. (${n} au carnet)`,
    netBflyToast: (nm, first, k, tot) => `🦋 ${nm} ! Relâché${first ? ` — nouvelle espèce au carnet (${k}/${tot})` : ` (${k}/${tot} espèces)`}.`,
    bflyName: (sp) => ({ pieride: "Une piéride", citron: "Un citron", azure: "Un azuré", vulcain: "Un vulcain", paon: "Un paon-du-jour", machaon: "Un machaon" })[sp] || "Un papillon",
    bflyShort: (sp) => ({ pieride: "Piéride", citron: "Citron", azure: "Azuré", vulcain: "Vulcain", paon: "Paon-du-jour", machaon: "Machaon" })[sp] || sp,
    marketNetTitle: "Le stand d'à côté",
    marketNetDesc: "Une épuisette à manche de frêne : papillons, carpes (à relâcher).",
    marketNetBuy: (g) => `Acheter — ${g} or`,
    marketNetOwned: "Vous en avez une.",
    netLogTitle: "📓 Carnet",
    netLogCarp: (n) => `Carpes attrapées et relâchées : ${n}`,
    netLogBfly: (k, tot) => `Papillons : ${k}/${tot} espèces`,
    spyglassLines: [
  ...
  en: {
    ...
    // 2026-09-12: selling an owned horse, from the shop.
    shopHorseOwnedHeader: "Owned horses",
    horseCoatName: (coat) => coat === "black" ? "Black horse" : coat === "white" ? "White horse" : "Bay horse",
    horseOwnedSub: (price) => `Bought for ${price} gold.`,
    horseMountedTag: "Mounted — cannot be sold",
    sellHorseBtn: (price) => `Sell (${price} gold)`,
    ...
    toolUsage: {
      ...
      scythe: "Trims wild bushes (you then walk through them without slowing down), then clears a trimmed bush to free the tile and gather a few twigs.",
    },
    ...
    devTeleportName: (k) => ({
      ...
      townChurch: "⛪ Valley Town — the church (forecourt)",
    ...
    // 2026-09-26 — the cat you feed, the net and the notebook.
    catName: (c) => ({ roux: "the ginger cat", noir: "the black cat", tricolore: "the calico cat" })[c] || "the cat",
    catMilkToast: (nm, n, max) => n >= max ? `🥛 ${nm[0].toUpperCase() + nm.slice(1)} laps up the milk and rubs against you.` : `🥛 ${nm[0].toUpperCase() + nm.slice(1)} laps up every last drop. (${n}/${max})`,
    catAdoptedToast: (nm) => `💛 ${nm[0].toUpperCase() + nm.slice(1)} has adopted you. It will come and see you in town.`,
    catGiftToast: (nm, fish) => `🐟 ${nm[0].toUpperCase() + nm.slice(1)} drops a ${fish.toLowerCase()} at your feet. Thanks…?`,
    catMilkToday: (nm) => `${nm[0].toUpperCase() + nm.slice(1)} has had its milk today.`,
    catMilkNone: "You'd need some milk (cow's or goat's).",
    promptCatMilk: "[E] Give the cat some milk",
    promptNetBfly: "[E] Swing the net (butterfly)",
    promptNetCarp: "[E] Swing the net (carp)",
    netBoughtToast: (g) => `🥅 A net! (−${g} gold). Get close to a butterfly or a carp, then E.`,
    netHaveToast: "You already have a net.",
    netNoGoldToast: (g) => `The till needs ${g} gold for the net.`,
    netMissToast: (carp) => carp ? "💦 Missed — the carp dives." : "💨 Missed — it got away.",
    netCarpToast: (n) => `🐟 A carp in the net! Released at once — the town hall forbids fishing them. (${n} in the notebook)`,
    netBflyToast: (nm, first, k, tot) => `🦋 ${nm}! Released${first ? ` — new species in the notebook (${k}/${tot})` : ` (${k}/${tot} species)`}.`,
    bflyName: (sp) => ({ pieride: "A cabbage white", citron: "A brimstone", azure: "A common blue", vulcain: "A red admiral", paon: "A peacock", machaon: "A swallowtail" })[sp] || "A butterfly",
    bflyShort: (sp) => ({ pieride: "Cabbage white", citron: "Brimstone", azure: "Common blue", vulcain: "Red admiral", paon: "Peacock", machaon: "Swallowtail" })[sp] || sp,
    marketNetTitle: "The stall next door",
    marketNetDesc: "An ash-handled net: butterflies, carp (to be released).",
    marketNetBuy: (g) => `Buy — ${g} gold`,
    marketNetOwned: "You have one.",
    netLogTitle: "📓 Notebook",
    netLogCarp: (n) => `Carp caught and released: ${n}`,
    netLogBfly: (k, tot) => `Butterflies: ${k}/${tot} species`,
    spyglassLines: [
