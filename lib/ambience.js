...
function fadeTo(el, target, ms, onDone) {
  ...
  function step(now) {
    ...
    /* ⚠️⚠️ ZIP 439 — LE BORNAGE N'EST PAS DE LA PRUDENCE, IL CORRIGE UNE
       EXCEPTION RÉELLE. Trouvé dans la console en allant regarder le rendu du
       lac : `IndexSizeError: volume (-0.0000453333) is outside the range
       [0, 1]`. L'interpolation part de `el.volume` et vise `target` ; les deux
       sont bornés, mais leur produit par `p` en virgule flottante sort du
       segment d'un dix-millième — assez pour que le navigateur JETTE.
       ⚠️ ET CE N'EST PAS UN AVERTISSEMENT ANODIN : `step` est appelé par
       `requestAnimationFrame`, donc l'exception tombe DANS une image, et elle
       tue la fin de cette image. C'est très exactement le piège n°1 de
       `CLAUDE.md` §4 (« l'exception ne s'arrête pas là où elle tombe : elle
       emporte tout ce que la frame devait encore dessiner »), rencontré ici
       dans le son plutôt que dans le rendu. Elle interrompt aussi la chaîne de
       `requestAnimationFrame` du fondu, qui reste donc à mi-course. */
