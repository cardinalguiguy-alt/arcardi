...
// jamais rien).
// `launchAtIso` (optionnel, correctif latence hôte 2026-07) : l'appelant
// (`launch` dans page.js) calcule lui-même l'horodatage cible AVANT d'appeler
// cette fonction, l'applique tout de suite en local (l'hôte n'attend plus le
// round-trip réseau pour voir sa propre transition démarrer), et le repasse
// ici pour que la valeur ÉCRITE en base soit EXACTEMENT celle que l'hôte
// utilise déjà localement — zéro dérive entre l'animation de l'hôte et celle
// des invités. Sans argument, comportement inchangé (rétrocompatible).
export async function launchGame(roomId, gameId, launchAtIso) {
  const launchAt = launchAtIso || new Date(Date.now() + GAME_LAUNCH_BUFFER_MS).toISOString();
...
// `openAtIso` (optionnel) : même principe que launchGame ci-dessus — l'hôte
// calcule et applique l'horodatage localement AVANT l'appel pour ouvrir sa
// porte sans attendre le réseau, et le repasse ici pour que la base écrive la
// même valeur. Sans argument, comportement inchangé (rétrocompatible).
export async function launchStage(roomId, openAtIso) {
  const openAt = openAtIso || new Date(Date.now() + STAGE_LAUNCH_BUFFER_MS).toISOString();
