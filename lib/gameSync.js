...
export async function resetRoomToLobby(roomId) {
  const { error } = await supabase
    .from("rooms")
    .update({ status: "lobby", current_game: null, game_state: null, launch_at: null })
    .eq("id", roomId);
  if (error) {
...
export async function launchGame(roomId, gameId) {
  ...
  const { error } = await supabase
    .from("rooms")
    .update({ status: "playing", current_game: gameId, launch_at: launchAt })
    .eq("id", roomId);
  if (error) {
    // Colonne launch_at pas encore migrée (upgrade-003.sql) : on retente
    // sans elle pour ne jamais bloquer le lancement, migration ou pas —
    // même filet de sécurité que resetRoomToLobby ci-dessus.
