...
// Mot Mystère — deux sons DISTINCTS à la révélation des lettres (demande
// 2026-07) : le VERT (lettre bien placée) sonne haut et lumineux, le JAUNE
// (lettre présente ailleurs) sonne plus bas et plus doux — on distingue les
// deux à l'oreille sans regarder la grille. Volume contenu : plusieurs
// lettres peuvent se révéler à la suite (une par ~230 ms).
export function playWordleGreen() {
  playNoteSequence([
    { freq: 1046, atMs: 0, durMs: 150, gain: 0.11 },   // do6
    { freq: 1318, atMs: 70, durMs: 190, gain: 0.10 },  // mi6 — petite tierce montante, "gagné"
  ], { type: "triangle" });
}
export function playWordleYellow() {
  playNoteSequence([
    { freq: 660, atMs: 0, durMs: 170, gain: 0.10 },    // mi5 seul, mat et plus grave — "presque"
  ], { type: "sine" });
}
